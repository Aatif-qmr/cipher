...
def send_telegram_health_report(results: list, timestamp: str):
    ...
    emoji = (
        ...
        if len(failed) == 0 or all(not f.get("critical", False) for f in failed)
        else "🚨"
    )
    ...
    if QNT_TELEGRAM_TOKEN and QNT_TELEGRAM_CHAT_ID:
        try:
            requests.post(
                ...
                json={"chat_id": QNT_TELEGRAM_CHAT_ID, "text": text, "parse_mode": "HTML"},
                timeout=10,
            )
        except Exception as e:
...
def check_freqtrade_processes():
    ...
    message = " | ".join(
        [f"{k[10:] if k.startswith('freqtrade_') else k}: {v}" for k, v in status_map.items()]
    )

    if all_running:
        if restart_attempts:
            recovered_msg = ", ".join(
                [f"{k}: recovered in {v} attempts" for k, v in restart_attempts.items()]
            )
            return {
                "name": "Freqtrade Processes",
                "status": "WARN",
                "message": f"Recovered: {recovered_msg} | {message}",
                "critical": False,
            }
        return {
            "name": "Freqtrade Processes",
            "status": "PASS",
            "message": f"All {len(programs)} instances running",
            "critical": True,
        }
...
def check_freqtrade_apis():
    ...
    if all_ok:
        return {
            "name": "Freqtrade APIs",
            "status": "PASS",
            "message": f"All {len(ports)} APIs responding",
            "critical": True,
        }
...
def check_sentiment_freshness():
    ...
    if not path.exists():
        return {
            "name": "Sentiment Freshness",
            "status": "FAIL",
            "message": "Score file missing",
            "critical": True,
        }
    try:
        with open(path) as f:
...
def check_m2_reachable():
    try:
        ...
        if res.returncode == 0:
            return {
                "name": "M2 Node Reachable",
                "status": "PASS",
                "message": f"IP {M2_IP} responding",
                "critical": True,
            }
        return {
            "name": "M2 Node Reachable",
            "status": "FAIL",
            "message": f"IP {M2_IP} unreachable",
            "critical": True,
        }
    except Exception as e:
        return {
...
def check_disk_space():
    ...
    return {
        "name": "Disk Space",
        "status": "WARN",
        "message": f"Low space: {free_gb:.1f} GB",
        "critical": False,
    }
...
def check_log_sizes():
    ...
    if total_size < 500:
        return {
            "name": "Log Sizes",
            "status": "PASS",
            "message": f"{total_size:.1f} MB total",
            "critical": False,
        }
    return {
        "name": "Log Sizes",
        "status": "WARN",
        "message": f"Logs large: {total_size:.1f} MB",
        "critical": False,
    }
...
def check_qnt_status():
    ...
    try:
        result = subprocess.run(
            [GEMINI_BIN, "--version"], capture_output=True, text=True, timeout=10
        )
    ...
    try:
        result = subprocess.run(
            [
                "ssh",
                f"{m2_user}@{m2_ip}",
                f"/Users/{m2_user}/cipher/venv/bin/python",
                f"/Users/{m2_user}/cipher/qnt/shadow/resource_monitor.py",
            ],
            capture_output=True,
            text=True,
            timeout=30,
...
def check_thesis_freshness() -> dict:
    ...
    if missing:
        return {
            "name": "Thesis Files",
            "status": "WARN",
            "message": f"Missing thesis for: {', '.join(missing)}",
            "critical": False,
        }
    if stale:
        return {
            "name": "Thesis Files",
            "status": "WARN",
            "message": f"Stale thesis: {', '.join(stale)}",
            "critical": False,
        }
...
def run_all():
    ...
    for fn in checks:
        try:
            ...
            results.append(
                {"name": name, "status": "FAIL", "message": f"Exception: {e}", "critical": True}
            )
