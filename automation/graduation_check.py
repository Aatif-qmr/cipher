...
def _evaluate(strategy: str) -> dict:
    ...
    if m["n"] < th["min_trades"]:
        ...
        if m["worst_loss"] < -th["max_drawdown"]:
            fails.append(
                f"worst loss {m['worst_loss'] * 100:.1f}% > -{th['max_drawdown'] * 100:.0f}% limit"
            )
...
def run() -> dict:
    ...
    core_passing = all(results[s]["verdict"] == "PASS" for s in core_strategies)
    all_data_present = all(results[s]["verdict"] != "INSUFFICIENT_DATA" for s in core_strategies)
    ...
    with open(LOG_PATH, "a") as f:
        f.write(
            f"[{now.isoformat()}] weeks={weeks_passing} core_pass={core_passing} graduated={graduated}\n"
        )
        for s, r in results.items():
            f.write(
                f"  {s}: {r['verdict']} n={r['metrics']['n']} wr={r['metrics']['win_rate']:.2f}\n"
            )
...
if __name__ == "__main__":
    ...
    for s, r in out["results"].items():
        m = r["metrics"]
        print(
            f"  {s}: {r['verdict']:20s}  n={m['n']:3d}  wr={m['win_rate']:.1%}  ev={m['expectancy'] * 100:+.2f}%"
        )
