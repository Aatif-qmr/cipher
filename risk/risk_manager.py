...
def _fetch_balance(port, ip, user, pwd):
    try:
        r = requests.get(f"http://{ip}:{port}/api/v1/balance", auth=(user, pwd), timeout=1.5)
...
def check_daily_drawdown(
    ...
    drawdown_pct = (
        ...
        else _compute_drawdown_pct(current_balance, start_of_day_balance)
    )

    # SANITY CHECK: If drawdown > 50% something is wrong with reading, not actual loss
    if drawdown_pct > 50.0:
        logging.warning(
            f"Impossible daily drawdown {drawdown_pct:.1f}% detected. Skipping alert/block."
        )
        return True
...
def check_weekly_drawdown(
    ...
    drawdown_pct = (
        precomputed_dd
        if precomputed_dd is not None
        else _compute_drawdown_pct(current_balance, start_of_week_balance)
    )

    # SANITY CHECK: If drawdown > 50% something is wrong with reading
    if drawdown_pct > 50.0:
        logging.warning(
            f"Impossible weekly drawdown {drawdown_pct:.1f}% detected. Skipping alert/block."
        )
        return True
...
def check_position_size(
    trade_amount_usdt: float, total_balance_usdt: float, max_pct: float = 10.0
) -> bool:
...
def run_all_checks(
    ...
    if start_of_day_balance is None or start_of_week_balance is None:
        try:
            with open(BASE_DIR / "risk/balance_state.json") as f:
                state = json.load(f)
            start_of_day_balance = start_of_day_balance or state.get(
    ...
    drawdowns = _batch_compute_drawdowns(
        [current_balance, current_balance], [start_of_day_balance, start_of_week_balance]
    )
    daily_dd, weekly_dd = drawdowns[0], drawdowns[1]

    checks = {
        "macro_headwinds": check_macro_headwinds(),
        "daily_drawdown": check_daily_drawdown(
            current_balance, start_of_day_balance, precomputed_dd=daily_dd
        ),
        "weekly_drawdown": check_weekly_drawdown(
            current_balance, start_of_week_balance, precomputed_dd=weekly_dd
        ),
