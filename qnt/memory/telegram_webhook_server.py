...
class TelegramWebhookHandler(BaseHTTPRequestHandler):
    ...
    def handle_reply(self, text: str, chat_id: str):
        ...
        try:
            ...
            mock_update = {"message": {"chat": {"id": chat_id}, "text": text}}
...
def run_polling():
    ...
    while True:
        try:
            res = requests.get(
                f"{API_URL}/getUpdates", params={"offset": offset, "timeout": 30}, timeout=40
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--setup-only", action="store_true", help="Only setup webhook, don't run server"
    )
