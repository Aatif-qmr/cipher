...
describe('useFolderTrust', () => {
  ...
  it('should send a message if the folder is untrusted', async () => {
    ...
    expect(addItem).toHaveBeenCalledWith(
      {
        text: 'This folder is untrusted, project settings, hooks, MCPs, and QNT.md files will not be applied for this folder.\nUse the `/permissions` command to change the trust level.',
        type: 'info',
      },
      expect.any(Number),
    );
  ...
  it('should emit feedback on failure to set value', async () => {
    ...
    expect(emitFeedbackSpy).toHaveBeenCalledWith(
      'error',
      'Failed to save trust settings. Exiting qnt.',
    );
