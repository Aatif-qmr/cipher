  ...
  describe('/memory list', () => {
    ...
    beforeEach(() => {
      ...
      vi.mocked(listMemoryFiles).mockImplementation((config) => {
        ...
        let content;
        if (fileCount > 0) {
          content = `There are ${fileCount} QNT.md file(s) in use:\n\n${filePaths.join('\n')}`;
        } else {
          content = 'No QNT.md files in use.';
        }
        return {
          type: 'message',
          messageType: 'info',
    ...
    it('should display a message if no QNT.md files are found', async () => {
    ...
    it('should display the file count and paths if they exist', async () => {
      ...
      const filePaths = ['/path/one/QNT.md', '/path/two/QNT.md'];
      ...
      expect(mockContext.ui.addItem).toHaveBeenCalledWith(
        {
          type: MessageType.INFO,
          text: `There are 2 QNT.md file(s) in use:\n\n${filePaths.join('\n')}`,
        },
        expect.any(Number),
      );
    });
