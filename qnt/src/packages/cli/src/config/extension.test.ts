...
describe('extension tests', () => {
  ...
  beforeEach(() => {
    ...
    tempHomeDir = getRealPath(
      fs.mkdtempSync(path.join(os.tmpdir(), 'qnt-test-home-')),
    );
    tempWorkspaceDir = getRealPath(
      fs.mkdtempSync(path.join(tempHomeDir, 'qnt-test-workspace-')),
    );
  ...
  describe('loadExtensions', () => {
    ...
    it('should load context file path when QNT.md is present', async () => {
      createExtension({
        extensionsDir: userExtensionsDir,
        name: 'ext1',
        version: '1.0.0',
      ...
      expect(ext1?.contextFiles).toEqual([
        path.join(userExtensionsDir, 'ext1', 'QNT.md'),
      ]);
