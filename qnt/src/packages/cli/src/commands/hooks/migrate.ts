...
export async function handleMigrateFromClaude() {
  ...
  try {
    ...
    debugLogger.log('✓ Hooks successfully migrated to .qnt/settings.json');
    debugLogger.log(
      '\nMigration complete! Please review the migrated hooks in .qnt/settings.json',
    );
  } catch (error) {
...
export const migrateCommand: CommandModule = {
  command: 'migrate',
  describe: 'Migrate hooks from Claude Code to qnt',
  builder: (yargs) =>
  ...
  handler: async (argv) => {
    ...
    if (args.fromClaude) {
      ...
      debugLogger.log(
        'Usage: gemini hooks migrate --from-claude\n\nMigrate hooks from Claude Code to qnt format.',
      );
    }
    await exitCli();
  },
