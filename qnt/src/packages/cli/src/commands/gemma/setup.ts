...
async function handleSetup(argv: SetupArgs): Promise<number> {
  ...
  try {
    ...
    log(chalk.dim('    User (~/.qnt/settings.json): autoStartServer'));
    log(
      chalk.dim('    Workspace (.qnt/settings.json): enabled, classifier'),
    );
    ...
    logError(
      '  You can manually add the configuration to ~/.qnt/settings.json',
    );
  }
