...
describe('getInstallationInfo', () => {
  ...
  it('should detect Homebrew installation via execSync', () => {
    ...
    const cliPath = '/opt/homebrew/Cellar/qnt/1.0.0/bin/gemini';
    process.argv[1] = cliPath;

    mockedExecSync.mockImplementation((cmd) => {
      if (typeof cmd === 'string' && cmd.includes('brew --prefix qnt')) {
        return '/opt/homebrew/opt/qnt';
      }
    ...
    mockedRealPathSync.mockImplementation((p) => {
      ...
      if (p === '/opt/homebrew/opt/qnt') {
        return '/opt/homebrew/Cellar/qnt/1.0.0';
      }
      return String(p);
    });
    ...
    expect(mockedExecSync).toHaveBeenCalledWith(
      expect.stringContaining('brew --prefix qnt'),
      expect.anything(),
    );
    expect(info.packageManager).toBe(PackageManager.HOMEBREW);
    expect(info.isGlobal).toBe(true);
    expect(info.updateMessage).toBe(
      'Installed via Homebrew. Please update with "brew upgrade qnt".',
    );
  });
  ...
  it('should detect global pnpm installation', () => {
    const pnpmPath = `/Users/test/.pnpm/global/5/node_modules/.pnpm/some-hash/node_modules/@google/qnt/dist/index.js`;
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('pnpm add -g @google/qnt@latest');
  ...
  it('should detect global yarn installation', () => {
    const yarnPath = `/Users/test/.yarn/global/node_modules/@google/qnt/dist/index.js`;
    ...
    expect(info.updateCommand).toBe(
      'yarn global add @google/qnt@latest',
    );
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should detect global bun installation', () => {
    const bunPath = `/Users/test/.bun/install/global/node_modules/@google/qnt/dist/index.js`;
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('bun add -g @google/qnt@latest');
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should default to global npm installation for unrecognized paths', () => {
    ...
    expect(info.packageManager).toBe(PackageManager.NPM);
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('npm install -g @google/qnt@latest');
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should NOT detect Homebrew if qnt is installed in brew but running from npm location', () => {
    Object.defineProperty(process, 'platform', {
      value: 'darwin',
    });
    // Path looks like standard global NPM
    const cliPath =
      '/usr/local/lib/node_modules/@google/qnt/dist/index.js';
    process.argv[1] = cliPath;

    // Setup mocks
    mockedExecSync.mockImplementation((cmd) => {
      if (typeof cmd === 'string' && cmd.includes('brew list')) {
        return Buffer.from('qnt\n');
      }
