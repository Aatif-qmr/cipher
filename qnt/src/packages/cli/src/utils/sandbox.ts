...
export async function start_sandbox(
  ...
  try {
    if (config.command === 'sandbox-exec') {
      ...
      // if profile name is not recognized, look in user-level ~/.qnt first,
      // then fall back to project-level .qnt. path.basename() strips any
    ...
    // determine full path for qnt to distinguish linked vs installed setting
    ...
    // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under qnt repo
    //
    // note this can only be done with binary linked from qnt repo
    if (process.env['BUILD_SANDBOX']) {
      if (!gcPath.includes('qnt/packages/')) {
        throw new FatalSandboxError(
          'Cannot build sandbox using installed gemini binary; ' +
            'run `npm link ./packages/cli` under qnt repo to switch to linked binary.',
        );
      } else {
    ...
    if (!(await ensureSandboxImageIsPresent(command, image, cliConfig))) {
      const remedy =
        image === LOCAL_DEV_SANDBOX_IMAGE_NAME
          ? 'Try running `npm run build:all` or `npm run build:sandbox` under the qnt repo to build it locally, or check the image name and your network connection.'
          : 'Please check the image name, your network connection, or notify qnt-dev@google.com if the issue persists.';
      throw new FatalSandboxError(
    ...
    if (isIntegrationTest) {
      containerName = `qnt-integration-test-${randomBytes(4).toString(
        'hex',
      )}`;
