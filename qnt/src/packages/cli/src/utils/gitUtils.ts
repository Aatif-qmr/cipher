...
export const getLatestGitHubRelease = async (
  ...
  try {
    ...
    const endpoint = `https://api.github.com/repos/google-github-actions/run-qnt/releases/latest`;
    ...
    debugLogger.debug(
      `Failed to determine latest run-qnt release:`,
      error,
    );
    throw new Error(
      `Unable to determine the latest run-qnt release on GitHub.`,
    );
  }
};
