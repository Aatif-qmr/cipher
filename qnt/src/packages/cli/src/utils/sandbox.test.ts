  ...
  describe('start_sandbox', () => {
    ...
    it('should resolve custom seatbelt profile from user home directory', async () => {
      ...
      vi.mocked(fs.existsSync).mockImplementation((p) =>
        String(p).includes(
          path.join(homedir(), '.qnt', 'sandbox-macos-custom-test.sb'),
        ),
      );
      const config: SandboxConfig = createMockSandboxConfig({
        command: 'sandbox-exec',
    ...
    it('should fall back to project .qnt directory when user profile is missing', async () => {
      vi.mocked(os.platform).mockReturnValue('darwin');
      vi.stubEnv('SEATBELT_PROFILE', 'custom-test');
      vi.mocked(fs.existsSync).mockImplementation((p) => {
        const s = String(p);
        return (
          s.includes(path.join('.qnt', 'sandbox-macos-custom-test.sb')) &&
          !s.includes(path.join(homedir(), '.qnt'))
        );
      });
      const config: SandboxConfig = createMockSandboxConfig({
        command: 'sandbox-exec',
      ...
      expect(profileArg).toEqual(
        expect.stringContaining(
          path.join('.qnt', 'sandbox-macos-custom-test.sb'),
        ),
      );
    ...
    it('should mount volumes correctly', async () => {
      ...
      expect(spawn).toHaveBeenNthCalledWith(
        ...
        expect.arrayContaining([
          ...
          '--volume',
          expect.stringMatching(/[\\/]home[\\/]user[\\/]\.qnt/),
        ]),
        expect.any(Object),
      );
    });
    ...
    it('should handle networkAccess: false in Docker', async () => {
      ...
      expect(execSync).toHaveBeenCalledWith(
        expect.stringContaining('network create --internal qnt-sandbox'),
        expect.any(Object),
      );
      expect(spawn).toHaveBeenCalledWith(
        'docker',
        expect.arrayContaining(['--network', 'qnt-sandbox']),
        expect.any(Object),
      );
    });
  ...
  describe('gVisor (runsc)', () => {
    it('should use docker with --runtime=runsc on Linux', async () => {
      ...
      expect(spawn).toHaveBeenNthCalledWith(
        1,
        'docker',
        expect.arrayContaining(['images', '-q', 'qnt-sandbox']),
      );
