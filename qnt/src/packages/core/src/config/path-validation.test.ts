...
describe('Config Path Validation', () => {
  ...
  const globalGeminiDir = path.join(os.homedir(), '.qnt');
  ...
  it('should allow access to a file under ~/.qnt once that directory is added to the workspace', () => {
    // Use settings.json rather than QNT.md as the example: the latter is
