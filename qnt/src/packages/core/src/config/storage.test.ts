...
describe('Storage – getGlobalSettingsPath', () => {
  it('returns path to ~/.qnt/settings.json', () => {
...
describe('Storage - Security', () => {
  it('falls back to tmp for gemini but returns empty for agents if the home directory cannot be determined', () => {
    ...
    // .qnt falls back for backward compatibility
...
describe('Storage – additional helpers', () => {
  ...
  it('getMcpOAuthTokensPath returns ~/.qnt/mcp-oauth-tokens.json', () => {
    const expected = path.join(
      os.homedir(),
      GEMINI_DIR,
  ...
  it('getGlobalBinDir returns ~/.qnt/tmp/bin', () => {
    const expected = path.join(os.homedir(), GEMINI_DIR, 'tmp', 'bin');
    expect(Storage.getGlobalBinDir()).toBe(expected);
  });

  it('getProjectTempPlansDir returns ~/.qnt/tmp/<identifier>/plans when no sessionId is provided', async () => {
    await storage.initialize();
  ...
  it('getProjectTempPlansDir returns ~/.qnt/tmp/<identifier>/<sessionId>/plans when sessionId is provided', async () => {
  ...
  it('getProjectTempTrackerDir returns ~/.qnt/tmp/<identifier>/tracker when no sessionId is provided', async () => {
    await storage.initialize();
    const tempDir = storage.getProjectTempDir();
  ...
  it('getProjectTempTrackerDir returns ~/.qnt/tmp/<identifier>/<sessionId>/tracker when sessionId is provided', async () => {
    const sessionId = 'test-session-id';
    const storageWithSession = new Storage(projectRoot, sessionId);
    ProjectRegistry.prototype.getShortId = vi
      .fn()
...
describe('Storage - System Paths', () => {
  ...
  it('getSystemSettingsPath returns correct path based on platform (default)', () => {
    ...
    if (platform === 'darwin') {
      ...
      expect(result).toBe('C:\\ProgramData\\qnt\\settings.json');
    } else {
      expect(result).toBe('/etc/qnt/settings.json');
    }
  });
  ...
  it('getSystemPoliciesDir returns correct path based on platform and ignores env var', () => {
    ...
    if (platform === 'darwin') {
      ...
      expect(result).toBe('C:\\ProgramData\\qnt\\policies');
    } else {
      expect(result).toBe('/etc/qnt/policies');
    }
  });
});
