...
describe('Config Agents Reload Integration', () => {
  ...
  beforeEach(async () => {
    ...
    // Create the .qnt/agents directory structure
    await fs.mkdir(path.join(tmpDir, '.qnt', 'agents'), { recursive: true });
  });
  ...
  it('should unregister agents from the agent registry when they are disabled after being enabled', async () => {
    ...
    const agentPath = path.join(tmpDir, '.qnt', 'agents', `${agentName}.md`);
