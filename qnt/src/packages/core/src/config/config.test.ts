...
vi.mock('../tools/memoryTool', () => ({
  ...
  getCurrentGeminiMdFilename: vi.fn(() => 'QNT.md'), // Mock the original filename
  DEFAULT_CONTEXT_FILENAME: 'QNT.md',
  GEMINI_DIR: '.qnt',
}));
...
describe('Config JIT Initialization', () => {
  ...
  beforeEach(() => {
    ...
    mockMemoryContextManager = {
      ...
      getUserProjectMemory: vi.fn().mockReturnValue(''),
      getLoadedPaths: vi.fn().mockReturnValue(new Set(['/path/to/QNT.md'])),
  ...
  it('should initialize MemoryContextManager, load memory, and delegate to it when experimentalJitContext is enabled', async () => {
    ...
    expect(config.getGeminiMdFilePaths()).toEqual(['/path/to/QNT.md']);
  });
  ...
  describe('isMemoryV2Enabled', () => {
    ...
    it('should NOT add the global ~/.qnt directory to the workspace when enabled', async () => {
      // The prompt-driven memoryV2 mode does not broaden the workspace
      // to include the global ~/.qnt/ directory. Cross-project personal
      // preferences are routed to ~/.qnt/QNT.md via the surgical
    ...
    it('should allow isPathAllowed to write the global ~/.qnt/QNT.md file', async () => {
      // Surgical allowlist: when memoryV2 is on, the prompt routes
      // cross-project personal preferences to ~/.qnt/QNT.md, so the
    ...
    it('should NOT allow isPathAllowed to write other files under ~/.qnt/ (least privilege)', async () => {
      // The allowlist is surgical: only ~/.qnt/QNT.md is reachable.
