...
async function getDocsRoot(): Promise<string> {
  ...
  throw new Error('Could not find qnt documentation directory.');
}
  ...
  async execute({ abortSignal: _signal }: ExecuteOptions): Promise<ToolResult> {
    try {
      ...
      if (!this.params.path) {
        ...
        const resultContent = `Available qnt documentation files:\n\n${fileList}`;
...
/**
 * A tool that provides access to qnt's internal documentation.
