...
export async function getRipgrepPath(): Promise<string | null> {
  ...
  const candidatePaths = [
    // 1. Local Dev: packages/core/src/tools/ripGrep.ts -> packages/core/vendor/ripgrep
    path.resolve(__dirname, '../../vendor/ripgrep', binName),
    // 2. Dev/Dist layout: packages/core/dist/src/tools/ripGrep.js -> packages/core/vendor/ripgrep
    path.resolve(__dirname, '../../vendor/ripgrep', binName),
    // 3. Bundle layout: bundle/gemini.js -> bundle/vendor/ripgrep
    // If __dirname is inside bundle/, we might be at bundle/ or bundle/src/
    path.resolve(__dirname, 'vendor/ripgrep', binName),
    path.resolve(__dirname, '../vendor/ripgrep', binName),
    path.resolve(__dirname, '../../vendor/ripgrep', binName),
    // 4. Root vendor (if all else fails)
    path.resolve(process.cwd(), 'vendor/ripgrep', binName),
    path.resolve(process.cwd(), 'packages/core/vendor/ripgrep', binName),
    path.resolve(process.cwd(), 'bundle/vendor/ripgrep', binName),
  ];
...
> {
  ...
  private async performRipgrepSearch(options: {
    ...
    if (!no_ignore) {
      ...
      // Add .qntignore and custom ignore files support (if provided/mandated)
