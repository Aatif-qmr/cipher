...
describe('FileDiscoveryService', () => {
  ...
  describe('initialization', () => {
    ...
    it('should load .qntignore patterns even when not in a git repo', async () => {
  ...
  describe('precedence (.qntignore over .gitignore)', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
    });

    it('should un-ignore a file in .qntignore that is ignored in .gitignore', async () => {
    ...
    it('should un-ignore a directory in .qntignore that is ignored in .gitignore', async () => {
    ...
    it('should extend ignore rules in .qntignore', async () => {
    ...
    it('should use .qntignore rules if respectGitIgnore is false', async () => {
      await createTestFile('.gitignore', '*.txt');
      await createTestFile(GEMINI_IGNORE_FILE_NAME, '!important.txt\ntemp/');
      ...
      // .qntignore un-ignores important.txt (which wasn't ignored anyway)
      // and ignores temp/
      expect(filtered).toEqual(
  ...
  describe('custom ignore file', () => {
    ...
    it('should prioritize custom ignore patterns over .qntignore patterns in git repo', async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/');
      await createTestFile(GEMINI_IGNORE_FILE_NAME, '*.log');

      const customIgnoreName = '.customignore';
      // .qntignore ignores *.log, custom un-ignores debug.log
    ...
    it('should prioritize custom ignore patterns over .qntignore patterns in non-git repo', async () => {
      // No .git directory created
      await createTestFile(GEMINI_IGNORE_FILE_NAME, 'secret.txt');

      const customIgnoreName = '.customignore';
      // .qntignore ignores secret.txt, custom un-ignores it
  ...
  describe('getIgnoreFilePaths & getAllIgnoreFilePaths', () => {
    ...
    it('should return .qntignore path by default', () => {
      const service = new FileDiscoveryService(projectRoot);
      const paths = service.getIgnoreFilePaths();
      expect(paths).toEqual([path.join(projectRoot, GEMINI_IGNORE_FILE_NAME)]);
    });

    it('should not return .qntignore path if respectGeminiIgnore is false', () => {
      const service = new FileDiscoveryService(projectRoot, {
        respectGeminiIgnore: false,
      });
      const paths = service.getIgnoreFilePaths();
