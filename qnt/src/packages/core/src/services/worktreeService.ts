...
/**
 * Service for managing Git worktrees within qnt.
...
export function getWorktreePath(projectRoot: string, name: string): string {
  return path.join(projectRoot, '.qnt', 'worktrees', name);
}
...
export function isGeminiWorktree(
  ...
  try {
    ...
    const worktreesBaseDir = path.join(realProjectRoot, '.qnt', 'worktrees');
