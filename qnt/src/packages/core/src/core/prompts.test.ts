...
describe('Core System Prompt (prompts.ts)', () => {
    ...
    mockConfig = {
      ...
      storage: {
        ...
        getProjectTempTrackerDir: vi
          .fn()
          .mockReturnValue('/mock/.qnt/tmp/session/tracker'),
      },
  ...
  it('should use chatty system prompt for preview model', () => {
    vi.mocked(mockConfig.getActiveModel).mockReturnValue(PREVIEW_GEMINI_MODEL);
    const prompt = getCoreSystemPrompt(mockConfig);
    expect(prompt).toContain('You are qnt, an interactive CLI agent'); // Check for core content
    expect(prompt).toContain('- **User Hints:**');
  ...
  it('should append userMemory with separator when provided', () => {
    ...
    expect(prompt).toContain('# Contextual Instructions (QNT.md)');
    expect(prompt).toContain('<loaded_context>');
    expect(prompt).toContain(memory);
    expect(prompt).toContain('You are qnt, an interactive CLI agent'); // Ensure base prompt follows
  ...
  it.each([
    ...
    [
      undefined,
      'You are qnt, an interactive CLI agent',
