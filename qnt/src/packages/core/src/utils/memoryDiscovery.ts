...
export async function getUserProjectMemoryPaths(
  ...
  try {
    ...
    // Fall back to the legacy private QNT.md file if the project has not
...
/**
 * Traverses upward from startDir to stopDir, finding all QNT.md variants.
 *
...
/**
 * Loads hierarchical QNT.md files and concatenates their content.
...
export async function loadServerHierarchicalMemory(
  ...
  if (allFilePathsStringDeduped.length === 0) {
    debugLogger.debug(
      '[DEBUG] [MemoryDiscovery] No QNT.md files found in hierarchy of the workspace.',
    );
    return {
  ...
  if (allFilePaths.length === 0) {
    debugLogger.debug(
      '[DEBUG] [MemoryDiscovery] No unique QNT.md files found after deduplication by file identity.',
    );
    return {
      memoryContent: { global: '', extension: '', project: '' },
      fileCount: 0,
...
export async function loadJitSubdirectoryMemory(
  ...
  // path like /app/src/file.ts/QNT.md.
