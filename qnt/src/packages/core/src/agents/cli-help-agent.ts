...
const CliHelpReportSchema = z.object({
  answer: z
    .string()
    .describe('The detailed answer to the user question about qnt.'),
  sources: z
    .array(z.string())
...
/**
 * An agent specialized in answering questions about qnt itself,
...
export const CliHelpAgent = (
  ...
  description:
    'Specialized agent for answering questions about the qnt application. Invoke this agent for questions regarding CLI features, configuration schemas (e.g., policies), or instructions on how to create custom subagents. It queries internal documentation to provide accurate usage guidance.',
  inputConfig: {
    inputSchema: {
      type: 'object',
      properties: {
        question: {
          type: 'string',
          description: 'The specific question about qnt.',
        },
      },
      required: ['question'],
    },
  ...
  promptConfig: {
    query:
      'Your task is to answer the following question about qnt:\n' +
      '<question>\n' +
      '${question}\n' +
      '</question>',
    systemPrompt:
      "You are **CLI Help Agent**, an expert on qnt. Your purpose is to provide accurate information about qnt's features, configuration, and current state.\n\n" +
      '### Runtime Context\n' +
