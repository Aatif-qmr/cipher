...
const CLI_IDE_COMPANION_IDENTIFIER = 'Google.qnt-cli-vscode-ide-companion';
  ...
  try {
    ...
    if (
      ...
      const selection = await vscode.window.showInformationMessage(
        `A new version (${latestVersion}) of the qnt Companion extension is available.`,
...
export async function activate(context: vscode.ExtensionContext) {
  logger = vscode.window.createOutputChannel('qnt IDE Companion');
  ...
  if (
    ...
    void vscode.window.showInformationMessage(
      'qnt Companion extension successfully installed.',
    );
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('qnt.runGeminiCLI', async () => {
      const workspaceFolders = vscode.workspace.workspaceFolders;
      if (!workspaceFolders || workspaceFolders.length === 0) {
        vscode.window.showInformationMessage(
          'No folder open. Please open a folder to run qnt.',
        );
        return;
      }
      ...
      if (workspaceFolders.length === 1) {
        ...
        selectedFolder = await vscode.window.showWorkspaceFolderPick({
          placeHolder: 'Select a folder to run qnt in',
        });
      }
    ...
    vscode.commands.registerCommand('qnt.showNotices', async () => {
