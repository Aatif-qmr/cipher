...
def fetch_binance_macro():
    ...
    try:
        fund_res = requests.get(
            "https://fapi.binance.com/fapi/v1/premiumIndex?symbol=BTCUSDT", timeout=5
        )
        funding_rate = float(fund_res.json().get("lastFundingRate", 0))

        oi_res = requests.get(
            "https://fapi.binance.com/fapi/v1/openInterest?symbol=BTCUSDT", timeout=5
        )
