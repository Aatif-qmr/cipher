...
def fetch_calendar():
    ...
    try:
        ...
        cmd = f"bash {os.path.join(BASE_DIR, 'qnt/browser_bridge.sh')} page https://www.forexfactory.com/calendar"
...
def check_and_act():
    ...
    if imminent_high_risk and not adjustment_active:
        ...
        try:
            ...
            log_action(
                "risk_position_reduction", f"Reducing position sizes for {risk['level']} event"
            )
        ...
        if restore_at and datetime.now(timezone.utc) > datetime.fromisoformat(restore_at):
            ...
            send_notify(
                "Risk Management", "📅 Event window passed. Position sizes restored to normal."
            )
