...
def get_global_status_panel():
    try:
        ...
        for inst in instances:
            try:
                r = requests.get(
                    f"http://{os.getenv('M1_TAILSCALE_IP', '127.0.0.1')}:{inst['port']}/api/v1/ping",
                ...
                if r.status_code == 200:
                    running_count += 1
                    r_bal = requests.get(
                        f"http://{os.getenv('M1_TAILSCALE_IP', '127.0.0.1')}:{inst['port']}/api/v1/balance",
                        auth=HTTPBasicAuth(user, pwd),
                        timeout=0.5,
                    )
                    ...
                    r_stat = requests.get(
                        f"http://{os.getenv('M1_TAILSCALE_IP', '127.0.0.1')}:{inst['port']}/api/v1/status",
                        auth=HTTPBasicAuth(user, pwd),
                        timeout=0.5,
                    )
...
def get_market_intel_panel():
    try:
        ...
        content = Text.assemble(
            ...
            ("Funding:   ", "bold"),
            (f"{sent.get('component_scores', {}).get('funding', 0.0):.4f}\n", "dim"),
            ("Macro:     ", "bold"),
            ("🟢 LOW RISK", "green"),
        )
...
def get_trades_panel():
    try:
        ...
        for port in ports:
            try:
                r = requests.get(
                    f"http://{os.getenv('M1_TAILSCALE_IP', '127.0.0.1')}:{port}/api/v1/status",
                    auth=HTTPBasicAuth(user, pwd),
                    timeout=0.5,
                )
                if r.status_code == 200:
        ...
        if not all_trades:
            return Panel("No open positions", title="GLOBAL TRADES", expand=True)
...
def run_dashboard(once=False):
    while True:
        ...
        print(
            Panel(
                Text(
                    f"🤖 QNT Cockpit (Static) │ {now.strftime('%H:%M:%S IST')} │ Node: Intelligence (M2)",
                    justify="center",
                ),
        ...
        row1 = Columns(
            [get_global_status_panel(), get_market_intel_panel(), get_shield_panel()],
            equal=True,
            expand=True,
        )
        print(row1)
        ...
        print(
            Panel(
                Text("[Ctrl+C] to Exit │ 'qnt-dashboard' for full TUI", justify="center"),
                border_style="dim",
            )
        )
