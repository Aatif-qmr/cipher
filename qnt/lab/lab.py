...
def run_backtest(strategy_name, timerange='20240101-20260101', timeframe='1h'):
    ...
    try:
        ...
        metrics['total_profit_pct'] = float(re.search(r'Total profit %.*?([\d.-]+)', stdout).group(1)) if re.search(r'Total profit %.*?([\d.-]+)', stdout) else 0
        metrics['total_trades'] = int(re.search(r'Total trades.*?(\d+)', stdout).group(1)) if re.search(r'Total trades.*?(\d+)', stdout) else 0
    ...
    # Pass/Fail Criteria
    failed_criteria = []
    if metrics.get('total_profit_pct', 0) <= 0: failed_criteria.append("Profit <= 0%")
    if metrics.get('win_rate', 0) < 45: failed_criteria.append("Win rate < 45%")
    if metrics.get('max_drawdown', 0) > 20: failed_criteria.append("Max drawdown > 20%")
    if metrics.get('total_trades', 0) < 20: failed_criteria.append("Trades < 20")
...
def evolve_strategy(strategy_name, lookback_trades=20):
    ...
    qnt_path = '/Users/aatifquamre/.local/share/fnm/node-versions/v25.9.0/installation/bin/qnt'
    result = subprocess.run([qnt_path, '-p', prompt, '--output-format', 'text'], capture_output=True, text=True, timeout=180)
    ...
    send_notify("Lab — Strategy Evolved", f"🔬 Strategy evolved: {strategy_name}V2\nAnalyzed {len(df)} trades\nV2 saved to candidates/.")
    return v2_path
...
def deploy_strategy(strategy_file, force=False):
    ...
    run_on_m1(f"curl -s -X POST -u \"$FREQTRADE_UI_USERNAME:$FREQTRADE_UI_PASSWORD\" http://100.90.68.42:8080/api/v1/reload_config")
    
    send_notify("Lab — Strategy Deployed", f"✅ Strategy deployed: {strategy_name}\nNow active in Freqtrade.")
