...
def run() -> dict:
    ...
    for (strategy, pair, ctype, cval), counts in buckets.items():
        ...
        if ctype == "regime":
            ...
            print(
                f"  [constraint] {strategy}/{pair}: block regime={cval} "
            ...
            if threshold is not None:
                ...
                if threshold > existing:
                    ...
                    print(
                        f"  [constraint] {strategy}/{pair}: min_sentiment={threshold} "
                        f"(loss_rate={loss_rate:.0%}, n={total})"
                    )
    ...
    print(
        f"[constraint_extractor] Wrote {sum(len(v) for v in constraints.values())} "
